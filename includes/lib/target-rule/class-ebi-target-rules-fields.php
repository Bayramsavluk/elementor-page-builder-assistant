...
class EBI_Target_Rules_Fields {
	...
	public static function get_location_selections() {
		...
		$special_pages = array(
			'special-404'    => __( '404 Page', 'ea-page-builder-assistant' ),
			'special-search' => __( 'Search Page', 'ea-page-builder-assistant' ),
			'special-blog'   => __( 'Blog / Posts Page', 'ea-page-builder-assistant' ),
			'special-front'  => __( 'Front Page', 'ea-page-builder-assistant' ),
			'special-date'   => __( 'Date Archive', 'ea-page-builder-assistant' ),
			'special-author' => __( 'Author Archive', 'ea-page-builder-assistant' ),
		);

		if ( class_exists( 'WooCommerce' ) ) {
			$special_pages['special-woo-shop'] = __( 'WooCommerce Shop Page', 'ea-page-builder-assistant' );
		}

		$selection_options = array(
			'basic'         => array(
				'label' => __( 'Basic', 'ea-page-builder-assistant' ),
				'value' => array(
					'basic-global'    => __( 'Entire Website', 'ea-page-builder-assistant' ),
					'basic-singulars' => __( 'All Singulars', 'ea-page-builder-assistant' ),
					'basic-archives'  => __( 'All Archives', 'ea-page-builder-assistant' ),
				),
			),

			'special-pages' => array(
				'label' => __( 'Special Pages', 'ea-page-builder-assistant' ),
				'value' => $special_pages,
			),
		);
		...
		$selection_options['specific-target'] = array(
			'label' => __( 'Specific Target', 'ea-page-builder-assistant' ),
			'value' => array(
				'specifics' => __( 'Specific Pages / Posts / Taxonomies, etc.', 'ea-page-builder-assistant' ),
			),
		);
	...
	public static function get_user_selections() {
		$selection_options = array(
			'basic'    => array(
				'label' => __( 'Basic', 'ea-page-builder-assistant' ),
				'value' => array(
					'all'        => __( 'All', 'ea-page-builder-assistant' ),
					'logged-in'  => __( 'Logged In', 'ea-page-builder-assistant' ),
					'logged-out' => __( 'Logged Out', 'ea-page-builder-assistant' ),
				),
			),

			'advanced' => array(
				'label' => __( 'Advanced', 'ea-page-builder-assistant' ),
				'value' => array(),
			),
		);
	...
	public function admin_styles() {
		...
		$localize_vars = array(
			'ast_lang'      => $ebi_lang,
			'please_enter'  => __( 'Please enter', 'ea-page-builder-assistant' ),
			'please_delete' => __( 'Please delete', 'ea-page-builder-assistant' ),
			'more_char'     => __( 'or more characters', 'ea-page-builder-assistant' ),
			'character'     => __( 'character', 'ea-page-builder-assistant' ),
			'loading'       => __( 'Loading more results…', 'ea-page-builder-assistant' ),
			'only_select'   => __( 'You can only select', 'ea-page-builder-assistant' ),
			'item'          => __( 'item', 'ea-page-builder-assistant' ),
			'char_s'        => __( 's', 'ea-page-builder-assistant' ),
			'no_result'     => __( 'No results found', 'ea-page-builder-assistant' ),
			'searching'     => __( 'Searching…', 'ea-page-builder-assistant' ),
			'not_loader'    => __( 'The results could not be loaded.', 'ea-page-builder-assistant' ),
			'search'        => __( 'Search pages / post / categories', 'ea-page-builder-assistant' ),
	...
	public static function target_rule_settings_field( $name, $settings, $value ) {
		...
		$add_rule_label = isset( $settings['add_rule_label'] ) ? $settings['add_rule_label'] : __( 'Add Rule', 'ea-page-builder-assistant' );
		$saved_values   = $value;
		$output         = '';
		...
		$output .= '<option value="">' . __( 'Select', 'ea-page-builder-assistant' ) . '</option>';
	...
	public static function get_post_target_rule_options( $post_type, $taxonomy ) {
		...
		$all_posts                          = sprintf( __( 'All %s', 'ea-page-builder-assistant' ), $post_label );
		$post_option[ $post_name . '|all' ] = $all_posts;

		if ( 'pages' != $post_key ) {
			/* translators: %s post label */
			$all_archive                                = sprintf( __( 'All %s Archive', 'ea-page-builder-assistant' ), $post_label );
		...
		if ( in_array( $post_type->name, $taxonomy->object_type ) ) {
			...
			$tax_archive = sprintf( __( 'All %s Archive', 'ea-page-builder-assistant' ), $tax_label );
	...
	public static function generate_target_rule_selector( $type, $selection_options, $input_name, $saved_values, $add_rule_label ) {
		...
		if ( 'display' == $type ) {
			...
			$output .= '<a href="#" class="button">' . __( 'Add Exclusion Rule', 'ea-page-builder-assistant' ) . '</a>';
			$output .= '</div>';
		}
