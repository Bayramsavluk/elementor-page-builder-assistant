...
class EBI_Translator {
	...
	public function translate( $text, $target_lang = 'tr', $api_id = 'mymemory' ) {
		...
		if ( ! isset( $translation_settings['apis'][ $api_id ]['enabled'] ) || ! $translation_settings['apis'][ $api_id ]['enabled'] ) {
			return new \WP_Error( 'api_disabled', __( 'Seçilen çeviri API\'si aktif değil.', 'ea-page-builder-assistant' ) );
		}
		...
		switch ( $api_id ) {
			...
			default:
				return new \WP_Error( 'unknown_api', __( 'Bilinmeyen çeviri API\'si.', 'ea-page-builder-assistant' ) );
		}
	}
	...
	private function translate_mymemory( $text, $target_lang, $api_key = '' ) {
		...
		if ( ! $data || ! isset( $data['responseData'] ) || ! isset( $data['responseData']['translatedText'] ) ) {
			$error_message = isset( $data['responseStatus'] ) ? $data['responseStatus'] : __( 'Çeviri hatası', 'ea-page-builder-assistant' );
	...
	private function translate_libretranslate( $text, $target_lang, $api_url = '' ) {
		...
		if ( ! $data || ! isset( $data['translatedText'] ) ) {
			return new \WP_Error( 'translation_error', __( 'LibreTranslate çeviri hatası', 'ea-page-builder-assistant' ) );
		}
	...
	private function translate_deepl( $text, $target_lang, $api_key ) {
		if ( empty( $api_key ) ) {
			return new \WP_Error( 'missing_key', __( 'DeepL API key gerekli.', 'ea-page-builder-assistant' ) );
		}
		...
		if ( ! $data || ! isset( $data['translations'] ) || empty( $data['translations'] ) ) {
			return new \WP_Error( 'translation_error', __( 'DeepL çeviri hatası', 'ea-page-builder-assistant' ) );
		}
	...
	private function translate_microsoft( $text, $target_lang, $api_key ) {
		if ( empty( $api_key ) ) {
			return new \WP_Error( 'missing_key', __( 'Microsoft Translator API key gerekli.', 'ea-page-builder-assistant' ) );
		}
		...
		if ( ! $data || ! isset( $data[0] ) || ! isset( $data[0]['translations'] ) || empty( $data[0]['translations'] ) ) {
			return new \WP_Error( 'translation_error', __( 'Microsoft Translator çeviri hatası', 'ea-page-builder-assistant' ) );
		}
	...
	private function translate_yandex( $text, $target_lang, $api_key ) {
		if ( empty( $api_key ) ) {
			return new \WP_Error( 'missing_key', __( 'Yandex Translate API key gerekli.', 'ea-page-builder-assistant' ) );
		}
		...
		if ( ! $data || ! isset( $data['text'] ) || empty( $data['text'] ) ) {
			$error_message = isset( $data['message'] ) ? $data['message'] : __( 'Yandex çeviri hatası', 'ea-page-builder-assistant' );
			return new \WP_Error( 'translation_error', $error_message );
		}
