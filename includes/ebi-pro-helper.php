...
function ebi_pro_feature_notice( $feature = '' ) {
	...
	$message = $feature 
		? sprintf( __( '%s is a PRO feature.', 'elementor-bulk-importer' ), $feature )
		: __( 'This is a PRO feature.', 'elementor-bulk-importer' );
	
	printf(
		'<div class="notice notice-info"><p>%s <a href="%s" target="_blank">%s</a></p></div>',
		esc_html( $message ),
		esc_url( ebi_get_upgrade_url() ),
		esc_html__( 'Upgrade to PRO', 'elementor-bulk-importer' )
	);
}
