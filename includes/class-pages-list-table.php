...
class EBI_Pages_List_Table extends WP_List_Table {
	...
	public function get_columns() {
		$columns = array(
			...
			'title'        => __( 'Sayfa Adı', 'elementor-bulk-importer' ),
			'template_kit' => __( 'Şablon (Template Kit)', 'elementor-bulk-importer' ),
			'parent'       => __( 'Üst öge', 'elementor-bulk-importer' ),
			'template'     => __( 'Şablon', 'elementor-bulk-importer' ),
			'date'         => __( 'Tarih', 'elementor-bulk-importer' ),
		);
		...
		switch ( $column_name ) {
			...
			case 'template':
				...
				if ( empty( $page_template ) || 'default' === $page_template ) {
					return __( 'Varsayılan şablon', 'elementor-bulk-importer' );
				}
			...
			case 'date':
				...
				$time_ago = sprintf(
					...
					__( '%s önce', 'elementor-bulk-importer' ),
					human_time_diff( $post_timestamp, current_time( 'timestamp' ) )
				);
				$full_date = mysql2date( __( 'Y/m/d g:i:s a', 'elementor-bulk-importer' ), $item->post_date );
	...
	protected function column_title( $item ) {
		...
		$actions['edit'] = sprintf(
			...
			esc_url( $edit_link ),
			esc_attr( sprintf( __( '“%s” düzenle', 'elementor-bulk-importer' ), $title ) ),
			__( 'Düzenle', 'elementor-bulk-importer' )
		);
			...
			try {
				...
				if ( $elementor_edit_link ) {
					$actions['elementor'] = sprintf(
						...
						esc_attr( sprintf( __( '“%s” Elementor ile düzenle', 'elementor-bulk-importer' ), $title ) ),
						__( 'Elementor ile düzenle', 'elementor-bulk-importer' )
					);
				}
			} catch ( Exception $e ) {
		...
		if ( 'trash' === $item->post_status ) {
			if ( current_user_can( 'delete_post', $item->ID ) ) {
				$actions['untrash'] = sprintf(
					...
					esc_attr( sprintf( __( '"%s" geri yükle', 'elementor-bulk-importer' ), $title ) ),
					__( 'Geri Yükle', 'elementor-bulk-importer' )
				);
				$actions['delete'] = sprintf(
					'<a href="%s" class="submitdelete" aria-label="%s">%s</a>',
					esc_url( get_delete_post_link( $item->ID, '', true ) ),
					esc_attr( sprintf( __( '"%s" kalıcı olarak sil', 'elementor-bulk-importer' ), $title ) ),
					__( 'Kalıcı olarak sil', 'elementor-bulk-importer' )
				);
			}
		} else {
			...
			if ( current_user_can( 'edit_post', $item->ID ) ) {
				$actions['inline hide-if-no-js'] = sprintf(
					...
					esc_attr( sprintf( __( '"%s" (Quick Edit)', 'elementor-bulk-importer' ), $title ) ),
					__( 'Hızlı Düzenle', 'elementor-bulk-importer' )
				);
			}
			...
			if ( current_user_can( 'delete_post', $item->ID ) ) {
				$actions['trash'] = sprintf(
					...
					esc_attr( sprintf( __( 'Move "%s" to trash', 'elementor-bulk-importer' ), $title ) ),
					__( 'Trash', 'elementor-bulk-importer' )
				);
			}
		}
		...
		if ( is_post_type_viewable( $post_type_object ) && 'trash' !== $item->post_status ) {
			$actions['view'] = sprintf(
				...
				esc_attr( sprintf( __( 'View "%s"', 'elementor-bulk-importer' ), $title ) ),
				__( 'View', 'elementor-bulk-importer' )
			);
		}
		...
		$output = sprintf(
			...
			esc_url( $edit_link ),
			esc_attr( sprintf( __( 'Edit "%s"', 'elementor-bulk-importer' ), $title ) ),
			esc_html( $title )
		);
	...
	if ( 'trash' === $post_status ) {
			...
			if ( current_user_can( 'delete_posts' ) ) {
				$actions['untrash'] = __( 'Restore', 'elementor-bulk-importer' );
				$actions['delete'] = __( 'Delete Permanently', 'elementor-bulk-importer' );
			}
		} else {
			// Normal: only Move to Trash
			if ( current_user_can( 'delete_posts' ) ) {
				$actions['trash'] = __( 'Move to Trash', 'elementor-bulk-importer' );
			}
		}
	...
	protected function extra_tablenav( $which ) {
		...
		<div class="alignleft actions">
			...
			if ( ! empty( $months ) ) {
				?>
				<select name="m">
					<option value="0"><?php esc_html_e( 'Tüm tarihler', 'elementor-bulk-importer' ); ?></option>
					<?php echo $months; ?>
				</select>
				<input type="submit" name="filter_action" id="post-query-submit" class="button" value="<?php esc_attr_e( 'Süz', 'elementor-bulk-importer' ); ?>">
				<?php
			}
			?>
		</div>
	...
	private function get_months_dropdown() {
		...
		foreach ( $months as $arc_row ) {
			...
			$month_label = sprintf(
				...
				__( '%1$s %2$d', 'elementor-bulk-importer' ),
	...
	public function no_items() {
		esc_html_e( 'Henüz sayfa eklenmemiş.', 'elementor-bulk-importer' );
	}
