...
class EBI_Importer {
		...
		if ( post_type_exists( 'elementor_library' ) ) {
			...
			foreach ( $library_posts as $post ) {
				...
				$templates[] = array(
					...
					'kit_name'    => __( 'Elementor Library', 'ea-page-builder-assistant' ),
	...
	public function import_template_to_page( $page_title, $template, $options = array() ) {
		if ( empty( $page_title ) || empty( $template ) ) {
			return new \WP_Error( 'invalid_params', __( 'Sayfa adı ve şablon gerekli.', 'ea-page-builder-assistant' ) );
		}

        // Parse template ID (supports "library:POST_ID" or "KIT_ID:TEMPLATE_ID")
        $parts = explode( ':', $template );
        if ( count( $parts ) !== 2 ) {
            return new \WP_Error( 'invalid_template', __( 'Geçersiz şablon formatı.', 'ea-page-builder-assistant' ) );
        }
		...
		if ( ! class_exists( '\Template_Kit_Import\Backend\Template_Kits' ) ) {
			return new \WP_Error( 'plugin_missing', __( 'Template Kit Import eklentisi bulunamadı.', 'ea-page-builder-assistant' ) );
		}
		...
		if ( ! empty( $elementor_content ) ) {
			// Translate Elementor content if translation is enabled and PRO version
			if ( EBI_PRO_VERSION && isset( $options['translation'] ) && $options['translation']['enabled'] ) {
	...
	private function import_library_template_to_page( $page_title, $template_id, $options = array() ) {
		...
		if ( ! $template_post || 'elementor_library' !== $template_post->post_type ) {
			return new \WP_Error( 'invalid_template', __( 'Geçersiz Elementor şablonu.', 'ea-page-builder-assistant' ) );
		}
		...
		if ( ! empty( $elementor_content ) ) {
			// Translate if needed and PRO version
			if ( EBI_PRO_VERSION && isset( $options['translation'] ) && $options['translation']['enabled'] ) {
				$translation_api = isset( $options['translation']['api_id'] ) ? $options['translation']['api_id'] : 'mymemory';
				$target_lang = isset( $options['translation']['target_lang'] ) ? $options['translation']['target_lang'] : 'tr';
	...
	public function import_template_as_section( $template, $section_type = 'header', $title = '', $options = array() ) {
		if ( empty( $template ) ) {
			return new \WP_Error( 'invalid_params', __( 'Şablon gerekli.', 'ea-page-builder-assistant' ) );
		}
		...
		// Translate Elementor content if translation is enabled and PRO version (before post type conversion)
		if ( EBI_PRO_VERSION && isset( $options['translation'] ) && $options['translation']['enabled'] ) {
			$translation_api = isset( $options['translation']['api_id'] ) ? $options['translation']['api_id'] : 'mymemory';
			$target_lang = isset( $options['translation']['target_lang'] ) ? $options['translation']['target_lang'] : 'tr';
		...
		$section_names = array(
			'header'       => __( 'Header', 'ea-page-builder-assistant' ),
			'footer'       => __( 'Footer', 'ea-page-builder-assistant' ),
			'before_footer' => __( 'Before Footer', 'ea-page-builder-assistant' ),
			'404'          => __( '404 Page', 'ea-page-builder-assistant' ),
		);
