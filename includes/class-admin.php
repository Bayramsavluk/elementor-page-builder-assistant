...
class EBI_Admin {
	...
	add_menu_page(
			__( 'EA Page Builder Assistant', 'ea-page-builder-assistant' ),
			__( 'EA Page Builder Assistant', 'ea-page-builder-assistant' ),
			'manage_options',
			'ebi-settings',
	...
	add_submenu_page(
			'ebi-settings',
			__( 'Pages', 'ea-page-builder-assistant' ),
			__( 'Pages', 'ea-page-builder-assistant' ),
			'manage_options',
			'ebi-settings',
			array( $this, 'render_pages_page' )
		...
		add_submenu_page(
			'ebi-settings',
			__( 'Header/Footer Builder', 'ea-page-builder-assistant' ),
			__( 'Header & Footer', 'ea-page-builder-assistant' ),
			'edit_pages',
			'ebi-header-footer',
		...
		add_submenu_page(
			'ebi-settings',
			__( 'Settings', 'ea-page-builder-assistant' ),
			__( 'Settings', 'ea-page-builder-assistant' ),
			'manage_options',
			'ebi-settings-page',
	...
	public function render_pages_page() {
		...
		<div class="wrap ebi-admin-page">
			...
			<a href="#" class="page-title-action ebi-add-new-page"><?php esc_html_e( 'Add New', 'ea-page-builder-assistant' ); ?></a>
		...
		<ul class="subsubsub">
			<li class="all">
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=ebi-settings' ) ); ?>" class="<?php echo 'all' === $current_status || empty( $current_status ) ? 'current' : ''; ?>">
					<?php esc_html_e( 'All', 'ea-page-builder-assistant' ); ?> <span class="count">(<?php echo esc_html( $all_count ); ?>)</span>
				</a>
			</li>
			<?php if ( $draft_count > 0 || 'draft' === $current_status ) : ?>
				<li class="draft">
					 | <a href="<?php echo esc_url( add_query_arg( 'post_status', 'draft', admin_url( 'admin.php?page=ebi-settings' ) ) ); ?>" class="<?php echo 'draft' === $current_status ? 'current' : ''; ?>">
						<?php esc_html_e( 'Draft', 'ea-page-builder-assistant' ); ?> <span class="count">(<?php echo esc_html( $draft_count ); ?>)</span>
					</a>
				</li>
			<?php endif; ?>
			<?php if ( $trash_count > 0 || 'trash' === $current_status ) : ?>
				<li class="trash">
					 | <a href="<?php echo esc_url( add_query_arg( 'post_status', 'trash', admin_url( 'admin.php?page=ebi-settings' ) ) ); ?>" class="<?php echo 'trash' === $current_status ? 'current' : ''; ?>">
						<?php esc_html_e( 'Trash', 'ea-page-builder-assistant' ); ?> <span class="count">(<?php echo esc_html( $trash_count ); ?>)</span>
					</a>
				</li>
			<?php endif; ?>
		</ul>
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=ebi-settings' ) ); ?>">
				...
				if ( isset( $_GET['m'] ) ) {
					...
					$list_table->search_box( __( 'Search Pages', 'ea-page-builder-assistant' ), 'post' );
				$list_table->display();
				?>
			</form>
			...
			<div id="ebi-add-page-modal" class="ebi-modal" style="display:none;" 
				...
				<div class="ebi-modal-content" style="max-width: 1200px;">
					<div class="ebi-modal-header" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
						<h2 style="margin:0;"><?php esc_html_e( 'Add New Page', 'ea-page-builder-assistant' ); ?></h2>
						<div>
							<button type="button" class="button ebi-add-page-row"><?php esc_html_e( 'Add Row', 'ea-page-builder-assistant' ); ?></button>
					...
					<div class="ebi-modal-body">
						<form id="ebi-add-page-form">
							...
							<div id="ebi-page-hidden-prototypes" style="display:none;">
								<div class="ebi-proto-parent">
									...
									wp_dropdown_pages( array(
										...
										'id'               => 'ebi-proto-parent',
										'show_option_none' => __( 'None', 'ea-page-builder-assistant' ),
										'option_none_value' => '0',
										'class'            => 'regular-text',
									) );
									?>
								...
								<div class="ebi-proto-ptype">
									...
									<option value="default"><?php esc_html_e( 'Default Template', 'ea-page-builder-assistant' ); ?></option>
										<option value="elementor_canvas"><?php esc_html_e( 'Elementor Canvas', 'ea-page-builder-assistant' ); ?></option>
										<option value="elementor_header_footer"><?php esc_html_e( 'Elementor Full Width', 'ea-page-builder-assistant' ); ?></option>
										<?php
										$theme_templates = get_page_templates();
										foreach ( $theme_templates as $template_name => $template_filename ) {
											echo '<option value="' . esc_attr( $template_filename ) . '">' . esc_html( $template_name ) . ' (' . esc_html( __( 'Theme', 'ea-page-builder-assistant' ) ) . ')</option>';
										}
										?>
									</select>
								</div>
							...
							<button type="submit" class="button button-primary"><?php esc_html_e( 'Add All', 'ea-page-builder-assistant' ); ?></button>
								<button type="button" class="button ebi-modal-cancel"><?php esc_html_e( 'Cancel', 'ea-page-builder-assistant' ); ?></button>
							</div>
						</form>
	...
	public function render_header_footer_list_page() {
		...
		?>
		<div class="wrap ebi-admin-page">
			<h1 class="wp-heading-inline"><?php echo esc_html__( 'Header & Footer', 'ea-page-builder-assistant' ); ?></h1>
			<a href="<?php echo esc_url( admin_url( 'post-new.php?post_type=elementor-hf' ) ); ?>" class="page-title-action"><?php esc_html_e( 'Add New', 'ea-page-builder-assistant' ); ?></a>
			<a href="#" class="page-title-action ebi-add-new-hf"><?php esc_html_e( 'Add from Template', 'ea-page-builder-assistant' ); ?></a>
			<hr class="wp-header-end">
		...
		<ul class="subsubsub">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin.php?page=ebi-header-footer' ) ); ?>">
				...
				$wp_list_table->search_box( __( 'Search', 'ea-page-builder-assistant' ), 'post' );
	...
	public function filter_hf_bulk_actions( $actions ) {
		...
		if ( 'trash' === $post_status ) {
			// Trash mode
			return array(
				'untrash' => __( 'Restore', 'ea-page-builder-assistant' ),
				'delete'  => __( 'Delete Permanently', 'ea-page-builder-assistant' ),
			);
		} else {
			// Normal mode
			return array(
				'trash' => __( 'Move to Trash', 'ea-page-builder-assistant' ),
			);
		}
	}
	...
	private function get_imported_pages() {
		...
		foreach ( $pages as $page ) {
			...
			$result[] = array(
				...
				'parent'        => $parent_id,
				'page_template' => empty( $page_template ) || 'default' === $page_template ? __( 'Varsayılan şablon', 'ea-page-builder-assistant' ) : $page_template,
		...
		if ( strpos( $hook, 'ebi-settings' ) !== false || 
			...
			wp_localize_script(
				...
				array(
					...
					'strings'  => array(
						'success'      => __( 'Başarılı!', 'ea-page-builder-assistant' ),
						'error'        => __( 'Hata oluştu!', 'ea-page-builder-assistant' ),
						'loading'      => __( 'Yükleniyor...', 'ea-page-builder-assistant' ),
						'selectTemplate' => __( 'Lütfen bir şablon seçin.', 'ea-page-builder-assistant' ),
					),
				)
			);
		}
	...
	public function header_footer_posttype() {
		$labels = array(
			'name'               => esc_html__( 'Elementor Header & Footer Builder', 'ea-page-builder-assistant' ),
			'singular_name'      => esc_html__( 'Elementor Header & Footer Builder', 'ea-page-builder-assistant' ),
			'menu_name'          => esc_html__( 'Elementor Header & Footer Builder', 'ea-page-builder-assistant' ),
			'name_admin_bar'     => esc_html__( 'Elementor Header & Footer Builder', 'ea-page-builder-assistant' ),
			'add_new'            => esc_html__( 'Add New', 'ea-page-builder-assistant' ),
			'add_new_item'       => esc_html__( 'Add New', 'ea-page-builder-assistant' ),
			'new_item'           => esc_html__( 'New Template', 'ea-page-builder-assistant' ),
			'edit_item'          => esc_html__( 'Edit Template', 'ea-page-builder-assistant' ),
			'view_item'          => esc_html__( 'View Template', 'ea-page-builder-assistant' ),
			'all_items'          => esc_html__( 'View All', 'ea-page-builder-assistant' ),
			'search_items'       => esc_html__( 'Search Templates', 'ea-page-builder-assistant' ),
			'parent_item_colon'  => esc_html__( 'Parent Templates:', 'ea-page-builder-assistant' ),
			'not_found'          => esc_html__( 'No Templates found.', 'ea-page-builder-assistant' ),
			'not_found_in_trash' => esc_html__( 'No Templates found in Trash.', 'ea-page-builder-assistant' ),
		);
	...
	public function ehf_register_metabox() {
		add_meta_box(
			'ehf-meta-box',
			__( 'Elementor Header & Footer Builder Options', 'ea-page-builder-assistant' ),
			array(
				$this,
				'efh_metabox_render',
			),
			...
			<tbody>
				<tr class="hfe-options-row type-of-template">
					<td class="hfe-options-row-heading">
						<label for="ehf_template_type"><?php esc_html_e( 'Type of Template', 'ea-page-builder-assistant' ); ?></label>
					</td>
					<td class="hfe-options-row-content">
						<select name="ehf_template_type" id="ehf_template_type">
							<option value="" <?php selected( $template_type, '' ); ?>><?php esc_html_e( 'Select Option', 'ea-page-builder-assistant' ); ?></option>
							<option value="type_header" <?php selected( $template_type, 'type_header' ); ?>><?php esc_html_e( 'Header', 'ea-page-builder-assistant' ); ?></option>
							<option value="type_before_footer" <?php selected( $template_type, 'type_before_footer' ); ?>><?php esc_html_e( 'Before Footer', 'ea-page-builder-assistant' ); ?></option>
							<option value="type_footer" <?php selected( $template_type, 'type_footer' ); ?>><?php esc_html_e( 'Footer', 'ea-page-builder-assistant' ); ?></option>
							<option value="custom" <?php selected( $template_type, 'custom' ); ?>><?php esc_html_e( 'Custom Block', 'ea-page-builder-assistant' ); ?></option>
						</select>
					</td>
				</tr>
				...
				<tr class="hfe-options-row hfe-shortcode">
					<td class="hfe-options-row-heading">
						<label for="ehf_template_type"><?php esc_html_e( 'Shortcode', 'ea-page-builder-assistant' ); ?></label>
						<i class="hfe-options-row-heading-help dashicons dashicons-editor-help" title="<?php esc_attr_e( 'Copy this shortcode and paste it into your post, page, or text widget content.', 'ea-page-builder-assistant' ); ?>">
						</i>
					</td>
					<td class="hfe-options-row-content">
				...
				<tr class="hfe-options-row enable-for-canvas">
					<td class="hfe-options-row-heading">
						<label for="display-on-canvas-template">
							<?php esc_html_e( 'Enable Layout for Elementor Canvas Template?', 'ea-page-builder-assistant' ); ?>
						</label>
						<i class="hfe-options-row-heading-help dashicons dashicons-editor-help" title="<?php esc_attr_e( 'Enabling this option will display this layout on pages using Elementor Canvas Template.', 'ea-page-builder-assistant' ); ?>"></i>
					</td>
					<td class="hfe-options-row-content">
		...
		if ( $target_rules_class ) {
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e( 'Display On', 'ea-page-builder-assistant' ); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
						title="<?php echo esc_attr__( 'Add locations for where this template should appear.', 'ea-page-builder-assistant' ); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					...
					$target_rules_class::target_rule_settings_field(
						...
						array(
							'title'          => __( 'Display Rules', 'ea-page-builder-assistant' ),
							'value'          => '[{"type":"basic-global","specific":null}]',
							'tags'           => 'site,enable,target,pages',
							'rule_type'      => 'display',
							'add_rule_label' => __( 'Add Display Rule', 'ea-page-builder-assistant' ),
						),
						$include_locations
					);
					?>
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e( 'Do Not Display On', 'ea-page-builder-assistant' ); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
						title="<?php echo esc_attr__( 'Add locations for where this template should not appear.', 'ea-page-builder-assistant' ); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					$target_rules_class::target_rule_settings_field(
						...
						array(
							'title'          => __( 'Exclude On', 'ea-page-builder-assistant' ),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => __( 'Add Exclusion Rule', 'ea-page-builder-assistant' ),
							'rule_type'      => 'exclude',
						),
						$exclude_locations
					);
			...
			<tr class="bsf-target-rules-row hfe-options-row">
				<td class="bsf-target-rules-row-heading hfe-options-row-heading">
					<label><?php esc_html_e( 'User Roles', 'ea-page-builder-assistant' ); ?></label>
					<i class="bsf-target-rules-heading-help dashicons dashicons-editor-help" title="<?php echo esc_attr__( 'Display custom template based on user role.', 'ea-page-builder-assistant' ); ?>"></i>
				</td>
				<td class="bsf-target-rules-row-content hfe-options-row-content">
					<?php
					$target_rules_class::target_user_role_settings_field(
						...
						array(
							'title'          => __( 'Users', 'ea-page-builder-assistant' ),
							'value'          => '[]',
							'tags'           => 'site,enable,target,pages',
							'add_rule_label' => __( 'Add User Rule', 'ea-page-builder-assistant' ),
						),
						$users
					);
					?>
	...
	public function column_headings( $columns ) {
		...
		$columns['shortcode'] = __( 'Shortcode', 'ea-page-builder-assistant' );
		
		// Display Rules sütununu ekle
		$columns['elementor_hf_display_rules'] = __( 'Display Rules', 'ea-page-builder-assistant' );
	...
	public function page_column_headings( $columns ) {
		...
		foreach ( $columns as $key => $value ) {
			if ( 'date' === $key ) {
				$new_columns['template_kit'] = __( 'Şablon (Template Kit)', 'ea-page-builder-assistant' );
				$new_columns['parent']      = __( 'Üst öge', 'ea-page-builder-assistant' );
				$new_columns['template']    = __( 'Şablon', 'ea-page-builder-assistant' );
			}
		...
		switch ( $column_name ) {
			...
			case 'template':
				...
				if ( empty( $page_template ) || 'default' === $page_template ) {
					echo esc_html__( 'Varsayılan şablon', 'ea-page-builder-assistant' );
				} else {
	...
	public function add_pages_modal() {
		...
		<div id="ebi-add-page-modal-edit" class="ebi-modal" style="display:none;" 
			...
			<div class="ebi-modal-content" style="max-width: 1200px;">
				<div class="ebi-modal-header" style="display:flex;align-items:center;justify-content:space-between;gap:12px;">
					<h2 style="margin:0;">&nbsp;<?php esc_html_e( 'Yeni Sayfa Ekle', 'ea-page-builder-assistant' ); ?></h2>
					<div>
						<button type="button" class="button ebi-add-page-row-edit"><?php esc_html_e( 'Yeni Alan', 'ea-page-builder-assistant' ); ?></button>
						<span class="ebi-modal-close" style="margin-left:8px;cursor:pointer;">&times;</span>
					</div>
				</div>
				<div class="ebi-modal-body">
					<form id="ebi-add-page-form-edit">
						...
						<div id="ebi-page-hidden-prototypes-edit" style="display:none;">
							...
							<div class="ebi-proto-ptype">
								<select id="ebi-proto-template-type-edit" class="regular-text">
									<option value="default"><?php esc_html_e( 'Varsayılan şablon', 'ea-page-builder-assistant' ); ?></option>
									<option value="elementor_canvas"><?php esc_html_e( 'Elementor Canvas', 'ea-page-builder-assistant' ); ?></option>
									<option value="elementor_header_footer"><?php esc_html_e( 'Elementor Tam Genişlik', 'ea-page-builder-assistant' ); ?></option>
									<?php
									$theme_templates = get_page_templates();
									foreach ( $theme_templates as $template_name => $template_filename ) {
										echo '<option value="' . esc_attr( $template_filename ) . '">' . esc_html( $template_name ) . ' (' . esc_html( __( 'Tema', 'ea-page-builder-assistant' ) ) . ')</option>';
									}
									?>
								</select>
							</div>
						...
						<div class="ebi-modal-actions" style="margin-top:12px;display:flex;gap:8px;justify-content:flex-end;">
							<button type="submit" class="button button-primary"><?php esc_html_e( 'Tümünü Ekle', 'ea-page-builder-assistant' ); ?></button>
							<button type="button" class="button ebi-modal-cancel"><?php esc_html_e( 'İptal', 'ea-page-builder-assistant' ); ?></button>
						</div>
					</form>
	...
	public function render_settings_page() {
		...
		if ( isset( $_POST['ebi_save_settings'] ) && check_admin_referer( 'ebi_settings_nonce', 'ebi_settings_nonce' ) ) {
			$this->save_settings();
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Ayarlar başarıyla kaydedildi.', 'ea-page-builder-assistant' ) . '</p></div>';
		}
		...
		<div class="wrap ebi-settings-page">
			<h1><?php esc_html_e( 'Ayarlar', 'ea-page-builder-assistant' ); ?></h1>
		...
		<nav class="nav-tab-wrapper">
			<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'ebi-settings-page', 'tab' => 'translation' ), admin_url( 'admin.php' ) ) ); ?>" 
			   class="nav-tab <?php echo 'translation' === $current_tab ? 'nav-tab-active' : ''; ?>">
				<?php esc_html_e( 'Dil Ayarları', 'ea-page-builder-assistant' ); ?>
			...
			<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'ebi-settings-page', 'tab' => 'about' ), admin_url( 'admin.php' ) ) ); ?>" 
			   class="nav-tab <?php echo 'about' === $current_tab ? 'nav-tab-active' : ''; ?>">
				<?php esc_html_e( 'About', 'ea-page-builder-assistant' ); ?>
			</a>
		</nav>
			...
			<?php else : ?>
				<form method="post" action="">
					...
					<?php if ( 'translation' === $current_tab ) : ?>
						...
						<?php if ( EBI_PRO_VERSION ) : ?>
							<p class="submit">
								<input type="submit" name="ebi_save_settings" class="button button-primary" value="<?php esc_attr_e( 'Ayarları Kaydet', 'ea-page-builder-assistant' ); ?>">
							</p>
						<?php endif; ?>
					<?php endif; ?>
				</form>
	...
	private function render_about_tab() {
		...
		<div class="ebi-support-content" style="max-width: 800px; margin: 20px 0;">
			<div class="ebi-support-box" style="background: #fff; padding: 30px; border: 1px solid #ddd; border-radius: 8px; margin-bottom: 20px;">
				<h2 style="margin-top: 0;"><?php esc_html_e( 'About Plugin', 'ea-page-builder-assistant' ); ?></h2>
				<p><strong><?php esc_html_e( 'Version:', 'ea-page-builder-assistant' ); ?></strong> <?php echo esc_html( EBI_VERSION ); ?></p>
				<p><strong><?php esc_html_e( 'Author:', 'ea-page-builder-assistant' ); ?></strong> <a href="https://bayramsavluk.com" target="_blank">Bayram Şavluk</a></p>
				<p><strong><?php esc_html_e( 'Telegram:', 'ea-page-builder-assistant' ); ?></strong> <a href="https://t.me/bayramsavluk" target="_blank">@bayramsavluk</a></p>
				<p><strong><?php esc_html_e( 'GitHub:', 'ea-page-builder-assistant' ); ?></strong> <a href="https://github.com/bayramsavluk/elementor-page-builder-assistant" target="_blank">Repository</a></p>
				<p><?php esc_html_e( 'Streamline your Elementor workflow by bulk importing template kits to pages and header/footer sections.', 'ea-page-builder-assistant' ); ?></p>
			</div>
		</div>
		<?php
	}
	...
	private function render_translation_settings_tab( $saved_settings, $default_api ) {
		...
		<?php if ( ! EBI_PRO_VERSION ) : ?>
			...
			<div class="ebi-get-pro-card" style="max-width: 800px; margin: 30px 0;">
				<h3><?php esc_html_e( 'Upgrade to PRO', 'ea-page-builder-assistant' ); ?></h3>
				<p><?php esc_html_e( 'Translation features are available in PRO version. Unlock powerful automatic translation with multiple APIs and language support.', 'ea-page-builder-assistant' ); ?></p>
				...
				<p style="margin-top: 20px;">
					<a href="<?php echo esc_url( admin_url( 'admin.php?page=ebi-settings-pricing' ) ); ?>" target="_blank" class="button button-primary" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border: none; box-shadow: 0 2px 8px rgba(102, 126, 234, 0.3); padding: 10px 20px; font-size: 14px; border-radius: 8px;">
						<?php esc_html_e( 'Get PRO Version', 'ea-page-builder-assistant' ); ?>
					</a>
				</p>
			</div>
		<?php else : ?>
		...
		$apis = array(
			'libretranslate' => array(
				'name' => 'LibreTranslate',
				'description' => __( 'Açık kaynak çeviri servisi. Self-hosted veya API kullanımı. 100+ dil desteği.', 'ea-page-builder-assistant' ),
				'requires_key' => false,
				'key_placeholder' => __( 'Self-hosted için API URL girin (opsiyonel)', 'ea-page-builder-assistant' ),
			),
			'mymemory' => array(
				'name' => 'MyMemory Translation',
				'description' => __( 'API key olmadan: Günde 100 istek (anonim). Email ile: Günde 1,000 istek. API key ile: Daha yüksek limitler. 103 dil desteği.', 'ea-page-builder-assistant' ),
				'requires_key' => false,
				'key_placeholder' => __( 'API Key (opsiyonel) - Key olmadan da kullanılabilir', 'ea-page-builder-assistant' ),
			),
			'deepl' => array(
				'name' => 'DeepL API',
				'description' => __( 'Yüksek kalite. Ücretsiz: 500,000 karakter/ay.', 'ea-page-builder-assistant' ),
				'requires_key' => true,
				'key_placeholder' => __( 'API Key gerekli', 'ea-page-builder-assistant' ),
			),
			'microsoft' => array(
				'name' => 'Microsoft Translator (Azure)',
				'description' => __( 'Azure üzerinden. Ücretsiz: 2M karakter/ay.', 'ea-page-builder-assistant' ),
				'requires_key' => true,
				'key_placeholder' => __( 'Azure API Key gerekli', 'ea-page-builder-assistant' ),
			),
			'yandex' => array(
				'name' => 'Yandex Translate',
				'description' => __( '10M karakter/gün ücretsiz. API key gerekli.', 'ea-page-builder-assistant' ),
				'requires_key' => true,
				'key_placeholder' => __( 'API Key gerekli', 'ea-page-builder-assistant' ),
			),
			'argostranslate' => array(
				'name' => 'Argos Translate',
				'description' => __( 'LibreTranslate tabanlı açık kaynak. Self-hosted.', 'ea-page-builder-assistant' ),
				'requires_key' => false,
				'key_placeholder' => __( 'API URL (opsiyonel)', 'ea-page-builder-assistant' ),
			),
		);
		...
		<table class="form-table">
			<thead>
				<tr>
					<th width="150"><?php esc_html_e( 'API', 'ea-page-builder-assistant' ); ?></th>
					<th width="100"><?php esc_html_e( 'Aktif', 'ea-page-builder-assistant' ); ?></th>
					<th><?php esc_html_e( 'API Key / URL', 'ea-page-builder-assistant' ); ?></th>
					<th width="120"><?php esc_html_e( 'Varsayılan', 'ea-page-builder-assistant' ); ?></th>
				</tr>
			</thead>
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<?php else : ?>
							<span class="description"><?php esc_html_e( 'API key gerekmez', 'ea-page-builder-assistant' ); ?></span>
						<?php endif; ?>
					</td>
					<td>
						<input type="radio" 
		...
		<div id="ebi-add-hf-modal" class="ebi-modal" style="display:none;">
			<div class="ebi-modal-content ebi-modal-large">
				<div class="ebi-modal-header">
					<h2><?php esc_html_e( 'Add New Header/Footer', 'ea-page-builder-assistant' ); ?></h2>
					<span class="ebi-modal-close">&times;</span>
				</div>
				<div class="ebi-modal-body">
					<form id="ebi-add-hf-form">
						<div class="ebi-form-group">
							<label for="ebi-hf-title"><?php esc_html_e( 'Name', 'ea-page-builder-assistant' ); ?> <span class="required">*</span></label>
							<input type="text" id="ebi-hf-title" name="hf_title" class="regular-text" required>
						</div>
						<div class="ebi-form-group">
							<label for="ebi-hf-type"><?php esc_html_e( 'Type of Template', 'ea-page-builder-assistant' ); ?> <span class="required">*</span></label>
							<select id="ebi-hf-type" name="hf_type" class="regular-text" required>
								<option value=""><?php esc_html_e( 'Select...', 'ea-page-builder-assistant' ); ?></option>
								<option value="header"><?php esc_html_e( 'Header', 'ea-page-builder-assistant' ); ?></option>
								<option value="footer"><?php esc_html_e( 'Footer', 'ea-page-builder-assistant' ); ?></option>
								<option value="before_footer"><?php esc_html_e( 'Before Footer', 'ea-page-builder-assistant' ); ?></option>
								<option value="404"><?php esc_html_e( '404 Page', 'ea-page-builder-assistant' ); ?></option>
							</select>
						</div>
						<div class="ebi-form-group ebi-template-select-wrapper" style="display:none;">
							<label for="ebi-hf-template"><?php esc_html_e( 'Template', 'ea-page-builder-assistant' ); ?> <span class="required">*</span></label>
							<select id="ebi-hf-template" name="hf_template" class="ebi-template-select" required>
								<option value=""><?php esc_html_e( 'Select template...', 'ea-page-builder-assistant' ); ?></option>
							</select>
						</div>
						...
						<div class="ebi-form-group" style="margin-top: 15px;">
							...
							<p class="description"><?php esc_html_e( 'Enabling this option will display this layout on pages using Elementor Canvas Template.', 'ea-page-builder-assistant' ); ?></p>
						</div>
			...
			// Translation options - for Header/Footer
				...
				<div class="ebi-form-group" style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #ddd;">
					<?php if ( ! EBI_PRO_VERSION ) : ?>
						<div style="background: #f0f6fc; border: 1px solid #0073aa; border-radius: 4px; padding: 15px; margin-bottom: 15px;">
							<p style="margin: 0 0 10px; color: #0073aa; font-weight: 600;">
								<span style="font-size: 18px;">🔒</span> <?php esc_html_e( 'Translation is a PRO Feature', 'ea-page-builder-assistant' ); ?>
							</p>
							<p style="margin: 0 0 10px; font-size: 14px;">
									<?php esc_html_e( 'Upgrade to PRO to unlock automatic translation with 5 APIs and 70+ languages!', 'ea-page-builder-assistant' ); ?>
							</p>
					...
					<label style="display: block; margin-bottom: 10px;">
						...
						<strong><?php esc_html_e( 'Enable translation?', 'ea-page-builder-assistant' ); ?></strong>
					</label>
					
					<div id="ebi-hf-translation-options" style="display: none; margin-left: 25px; margin-top: 10px;">
								<?php if ( ! empty( $enabled_apis ) ) : ?>
									<label for="ebi-hf-translation-api"><?php esc_html_e( 'Translation API:', 'ea-page-builder-assistant' ); ?></label>
								...
								<?php else : ?>
									<p class="description" style="color: #d63638; margin-bottom: 10px;">
										<strong><?php esc_html_e( 'Warning:', 'ea-page-builder-assistant' ); ?></strong> 
										<?php esc_html_e( 'To use translation feature, you must first', 'ea-page-builder-assistant' ); ?> 
										<a href="<?php echo esc_url( admin_url( 'admin.php?page=ebi-settings-page&tab=translation' ) ); ?>" target="_blank">
											<?php esc_html_e( 'enable at least one translation API from the Settings page.', 'ea-page-builder-assistant' ); ?>
										</a>
									</p>
								...
								<label for="ebi-hf-translation-target-lang" style="display: block; margin-top: 10px;">
									<?php esc_html_e( 'Target language:', 'ea-page-builder-assistant' ); ?>
								</label>
					...
					<div class="ebi-modal-actions" style="margin-top: 20px;">
							<button type="submit" class="button button-primary"><?php esc_html_e( 'Add', 'ea-page-builder-assistant' ); ?></button>
							<button type="button" class="button ebi-modal-cancel"><?php esc_html_e( 'Cancel', 'ea-page-builder-assistant' ); ?></button>
						</div>
					</form>
