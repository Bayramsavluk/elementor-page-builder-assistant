...
(function($) {
	...
	console.log('EBI Admin JS Loaded');
...
// (moved to top-level)
	...
	/* Translation tab is now always accessible */
			...
			success: function(response) {
				if (response.success && response.data.templates) {
                    ...
                    list.forEach(function(t){
                        // Format: "Kit Name - Template Name"
                        const displayText = t.kit_name + ' - ' + t.name;
                        masterTemplateSelect.append($('<option></option>').val(t.full_id).text(displayText));
                    });
				}
			},
			error: function() {
...
function addPageRow(containerSelector) {
    ...
    // Check if PRO version - check both modals
    const $modal = $('#ebi-add-page-modal').length ? $('#ebi-add-page-modal') : $('#ebi-add-page-modal-edit');
    const isPro = $modal.attr('data-is-pro') === '1';
    ...
    // Translation controls in header - only show if PRO
    if (isPro) {
        ...
        // Show PRO message for free users
        const $proMessage = $('<div class="ebi-translation-pro-notice" style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:#f0f6fc;border:1px solid #0073aa;border-radius:4px;font-size:12px;"></div>');
        const $proBadge = $('<span style="background:#667eea;color:#fff;padding:3px 8px;border-radius:3px;font-weight:600;white-space:nowrap;">🔒 PRO</span>');
        const $proText = $('<span style="color:#555;">Translation feature - Upgrade to translate during import</span>');
        $proMessage.append($proBadge).append($proText);
        $headRight.append($proMessage);
    }
